...
$username=htmlentities(mysqli_real_escape_string($_POST['username']));
$password=mysqli_real_escape_string($_POST['password']);
if ($password==NULL) {
header ("Location: index.php?nopass");
exit();
}else{	
$query = mysqli_query($con,"SELECT username,password, imagen FROM usuarios WHERE username = '$username'") or die(mysql_error());
...
if($_POST['recordar']){
	...
	$query = mysqli_query($con,"UPDATE usuarios SET id_extreme='".$id_extreme."' WHERE username='".$username2."'") or die(mysql_error());
}
